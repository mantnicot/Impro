export const DEFAULT_OBJECTS = [
  "Mesa", "Silla", "Cama", "Puerta", "Espejo", "Lámpara", "Reloj", "Llave",
  "Teléfono", "Libro", "Lápiz", "Taza", "Plato", "Cuchara", "Tenedor",
  "Cuchillo", "Olla", "Sartén", "Botella", "Vaso", "Jarra", "Pan", "Queso",
  "Huevo", "Manzana", "Plátano", "Naranja", "Uvas", "Sandía", "Tomate",
  "Zanahoria", "Papa", "Lechuga", "Pizza", "Pastel", "Galleta", "Helado",
  "Café", "Leche", "Miel", "Arroz", "Sopa", "Jamón",
  "Sombrero", "Zapato", "Guante", "Bufanda", "Camisa", "Pantalón", "Vestido",
  "Gafas", "Anillo", "Collar", "Mochila", "Maleta", "Paraguas", "Bolsa",
  "Pelota", "Balón", "Globo", "Dado", "Trompo", "Cometa", "Muñeca", "Peluche",
  "Martillo", "Tijeras", "Destornillador", "Caja", "Cinta", "Cuerda",
  "Escoba", "Cubo", "Jabón", "Toalla", "Almohada", "Manta", "Peine", "Cepillo",
  "Guitarra", "Piano", "Flauta", "Tambor", "Violín", "Trompeta", "Acordeón",
  "Carro", "Bici", "Avión", "Barco", "Tren", "Cohete",
  "Maceta", "Vela", "Escalera", "Televisor", "Radio", "Computador", "Teclado",
  "Monitor", "Cámara", "Micrófono", "Audífonos", "Calculadora", "Regla",
  "Cuaderno", "Bolígrafo", "Goma", "Grapadora", "Pincel", "Pintura",
  "Espada", "Escudo", "Corona", "Trofeo", "Medalla", "Bandera", "Silbato",
  "Lupa", "Brújula", "Mapa", "Linterna", "Candado", "Ventilador", "Nevera",
  "Tostadora", "Cafetera", "Microondas", "Florero", "Sofá", "Cortina", "Alfombra",
  "Cuadro", "Marco", "Sobre", "Clip", "Moneda", "Rompecabezas",
  "Patines", "Skate", "Raqueta", "Sombrilla", "Botas", "Corbata", "Cinturón",
  "Fresa", "Limón", "Piña", "Mango", "Coco", "Brócoli", "Cebolla", "Ajo",
  "Champiñón", "Salchicha", "Tocino", "Mantequilla", "Yogurt", "Chocolate",
  "Caramelo", "Donut", "Waffle", "Taco", "Sandwich", "Hamburguesa",
  "Termo", "Lonchera", "Hervidor", "Colador", "Rallador", "Exprimidor",
  "Carretilla", "Pala", "Hacha", "Taladro", "Sierra", "Nuez",
  // Objetos creativos y menos comunes
  "Abanico", "Acordeón roto", "Aguja", "Alfiler", "Antena", "Arcoíris de juguete",
  "Balde", "Batería", "Bicicleta sin ruedas", "Billetera vacía", "Binocular",
  "Boomerang", "Broche", "Burro de planchar", "Cactus", "Caja fuerte",
  "Calendario", "Campana", "Canoa", "Caracol", "Cartón", "Casco", "Cascabel",
  "Cenicienta de plástico", "Cerradura", "Chal", "Chimenea", "Chinche", "Ciruela",
  "Clavija", "Claxon", "Cocodrilo de goma", "Cohete de papel", "Colgante",
  "Compás", "Cono de tránsito", "Contador", "Corcho", "Crucero de juguete",
  "Delfín inflable", "Diamante falso", "Diccionario", "Dinosaurio de peluche",
  "Disco de vinilo", "Domino", "Dron", "Ducha", "Efigie", "Embudo", "Encendedor",
  "Enchufe", "Escalera de mano", "Escoba voladora", "Escultura", "Esfera",
  "Espejuelos de sol", "Estetoscopio", "Estrella de mar", "Estufa", "Etiqueta",
  "Extintor", "Faro", "Farol", "Ficha de dominó", "Filtro de café", "Flamenco de jardín",
  "Flash", "Flecha", "Flipper", "Frisbee", "Gancho", "Ganzúa", "Gato de porcelana",
  "Gazebo", "Globo terráqueo", "Gnomo de jardín", "Gondola", "Gorra", "Grillete",
  "Guirnalda", "Hacha de juguete", "Helicóptero", "Hoz", "Imán", "Incubadora",
  "Inodoro", "Jaula", "Juego de mesa", "Kayak", "Kiwi", "Ladrillo", "Lágrima de plástico",
  "Lápiz labial", "Lazo", "Lego", "Lente", "Licra", "Lima de uñas", "Lirio",
  "Lupa gigante", "Máscara", "Megáfono", "Memoria USB", "Metrónomo", "Microscopio",
  "Mina de lápiz", "Miniatura", "Molinillo", "Moneda de chocolate", "Monociclo",
  "Morsa de peluche", "Navaja suiza", "Neumático", "Nido", "Ocarina", "Ojo de cerradura",
  "Olla mágica", "Orquídea", "Oso de peluche", "Ostra", "Palmera en maceta",
  "Paracaídas", "Parlante", "Peonía", "Periscopio", "Perro de cerámica", "Petaca",
  "Piano de juguete", "Pipa", "Pirámide", "Pistola de agua", "Pluma de ave",
  "Pollo de goma", "Portafolio", "Proyector", "Pulpo de peluche", "Pulsera",
  "Puzzle", "Raqueta rota", "Red de pesca", "Reloj de arena", "Remo", "Robot",
  "Rodillo", "Ruleta", "Saco de boxeo", "Salero", "Saxofón", "Semáforo de juguete",
  "Sirena", "Sobre cerrado", "Sombrero de copa", "Submarino de juguete", "Tablero",
  "Tambor militar", "Tarro", "Telescopio", "Tetera", "Tiara", "Tobogán", "Tortuga",
  "Trampolín", "Tridente", "Trineo", "Trompo de combate", "Tubería", "Turbante",
  "Urna", "Varita mágica", "Velero", "Ventana", "Violeta", "Yate", "Yoyo",
  "Zancos", "Zapato de tacón", "Zorro de peluche", "Ábaco", "Águila de madera",
  "Calcetín suelto", "Control remoto sin pilas", "Paraguas invertido", "Taza con agujero",
  "Pizza fría", "Audífono enredado", "Cargador roto", "Selfie stick", "Drone perdido",
  "Termo derramado", "Llave inglesa oxidada", "Mapa arrugado", "Pasaporte vencido",
  "Ticket de lotería", "Foto polaroid", "Carta sin abrir", "Dado trucado",
  "Moneda australiana", "Medalla olímpica falsa", "Capa de superhéroe", "Varita de director",
  "Telón rojo", "Micrófono de pie", "Foco de teatro", "Programa de mano",
  // Objetos colombianos
  "Arepa", "Empanada", "Ajiaco", "Bandeja paisa", "Changua", "Buñuelo", "Oblea",
  "Mazamorra", "Aguapanela", "Chocoramo", "Achiras", "Almojábana", "Pandebono",
  "Roscon", "Tamal", "Sancocho", "Lechona", "Fritanga", "Picada", "Mazorca",
  "Aguardiente", "Chicha", "Refajo", "Café tinto", "Greca", "Pitillo", "Bolsa de pan",
  "Sombrero vueltiao", "Ruana", "Carriel", "Poncho", "Mochila Wayuu", "Sombrero aguadeño",
  "Chiva", "Buseta", "Transmilenio", "Carro lechero", "Motocarro", "Bicitaxi",
  "Palco de feria", "Silleta", "Flores de Silleteros", "Vela de pueblo", "Petardo",
  "Acordeón vallenato", "Guacharaca", "Caja vallenata", "Tambora", "Tiple",
  "Ruana de lana", "Carroza", "Balde de arepa", "Olla de barro", "Caldero",
  "Panela", "Piloncillo", "Bocadillo veleño", "Bocadillo de guayaba", "Copo de nieve",
  "Carriel antioqueño", "Sombrero de paja tolimense", "Ruana cafetera", "Jugo de lulo",
  "Guanábana", "Curuba", "Feijoa", "Granadilla", "Chontaduro", "Uchuva", "Tomate de árbol",
  "Plátano verde", "Patacón", "Hogao", "Ají", "Limonada de coco", "Salpicón",
  "Raspao", "Chuzo", "Perro caliente con piña", "Almojábana con chocolate",
  "Libreta de calificaciones", "Escarapela", "Bandera tricolor", "Escudo de Colombia",
  "Ruana en la silla", "Carriel de cuero", "Sombrero campesino", "Botella de Pony Malta",
];

export const FAVORITES_LIST_ID = "favoritos";
export const FAVORITES_LIST_NAME = "Favoritos";
export const RANDOM_LIST_ID = "aleatorio";
export const RANDOM_LIST_NAME = "Aleatorio";

import { shuffledCopy } from "@/lib/shuffle";

export function getRandomObject(): string {
  const pool = shuffledCopy(DEFAULT_OBJECTS);
  return pool[0] ?? DEFAULT_OBJECTS[0];
}
