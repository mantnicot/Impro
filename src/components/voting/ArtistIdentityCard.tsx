...
function ArtistAvatar({ artist, compact = false }: { artist: Artist; compact?: boolean }) {
  const isFemale = artist.avatar_gender === "female";
  const color = artistColor(artist);
  const box = compact ? "h-14 w-14" : "h-20 w-20";
  const icon = compact ? "h-11 w-11" : "h-16 w-16";

  return (
    <div className={`flex ${box} shrink-0 items-center justify-center rounded-2xl border-4 border-gray-900 bg-white shadow-sm`}>
...
export function ArtistIdentityCard({ artist, children, compact = false, muted = false }: ArtistIdentityCardProps) {
  ...
  return (
    ...
    >
      <div className="absolute left-0 top-0 h-full w-1.5 sm:w-2" style={{ backgroundColor: color }} />
      <div className={`flex gap-3 ${compact ? "items-center" : "items-start sm:gap-4"}`}>
        <div className={compact ? "" : "pt-1"}>
          <ArtistAvatar artist={artist} compact={compact} />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <h3
            className={`break-words font-display font-black leading-tight text-gray-900 ${
          ...
          {!compact && (
            <>
              <p
                className="mt-1 inline-block rounded-full px-3 py-1 text-xs font-bold text-gray-900"
      ...
      {children && <div className={`relative ${compact ? "mt-3" : "mt-4"}`}>{children}</div>}
    </article>
  );
}
